...
const get_dashboard= expressAsyncHandler(async (req, res)=> {
    try {
        ...
        const [rows2]= await connection.execute("SELECT * FROM history INNER JOIN user ON history.user_id = user.user_id INNER JOIN book ON book.book_id = history.book_id WHERE is_borrow= 1")
