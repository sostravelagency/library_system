...
// 'add'関数は、ユーザーのカートに本を追加する
const cart= {

    add: expressAsyncHandler(async (req, res)=> {
        try {           
            // 本がすでにカートにあるかどうかを確認する
            ...
            if(rows1.length > 0) {
                // 本がすでにカートに存在する場合は、 'add: false'と'exist: true'を返す
                return res.status(200).json({add: false, exist: true})
            }
            // チェックアウト可能な本かどうかを確認する
    ...
    // 'get'関数は、ユーザーのカートにあるすべての本を取得する
    get: expressAsyncHandler(async (req, res)=> {
        try {
            // ユーザーのカートにあるすべての本を取得する
