...
const request = {
    ...
    action: expressAsyncHandler(async(req, res)=> {
        try {
            // eslint-disable-next-line
            const [rows] = await connection.execute("UPDATE history SET state= ?, time_approve= ?, is_borrow= ? WHERE history_id= ?", [req.body.status, new Date(), req.body?.is_borrow || 0, req.body.id])
